...
| `scripts/sync-upstream.sh` | Fetch upstream tag, re-apply our additive tree, re-tag OUR sync commit (upstream tag objects carry workflow files GITHUB_TOKEN cannot push). |
...
9. **GITHUB_TOKEN cannot push refs that change workflow files** (no `workflows`
   scope for App tokens). Upstream releases that touch `.github/` (rev6 did,
   via esr.yml) make the fetched upstream tag unpushable → sync re-tags at the
   sync commit, whose tree keeps our workflows byte-identical to abrp-main
   (the patcher is NOT re-run during sync; it runs when abrp-main is updated
