...
# abrp: openBalena has no balena_os/<fleet>-supervisor applications, so the
# upstream API lookup cannot resolve; pull from GHCR instead (public image,
# built by the repo's build-supervisor workflow)
if [ -n "${SUPERVISOR_VERSION}" ]; then
	_supervisor_image="ghcr.io/volkermauel/aarch64-supervisor:${SUPERVISOR_VERSION}"
	echo "Pulling ${_supervisor_image}"
